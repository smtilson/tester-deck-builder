...
@pytest.mark.asyncio
class TestDeckCardManagerAdd:
    """Integration tests for DeckCardManager add card to deck operations."""
    ...
    async def test_add_card_to_deck_success(self, db, single_deck_and_cards):
        ...
        result = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card_create)
    ...
    async def test_add_existing_card_increases_quantity(
        ...
        # Add card first time
        result1 = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card_create)
        assert result1.quantity == 2

        # Add same card again
        result2 = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card_create)
    ...
    async def test_add_multiple_different_cards(self, db, single_deck_and_cards):
        ...
        result1 = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card1)
        result2 = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card2)
...
@pytest.mark.skip("standard")
@pytest.mark.asyncio
class TestDeckCardManagerGet:
    """Integration tests for DeckCardManager get operations."""
    ...
    async def test_get_cards_for_deck_returns_all(self, db, single_deck_with_cards):
        ...
        result = await DeckCardManager.gets_for_deck(test_deck.id)
    ...
    async def test_get_cards_for_empty_deck(self, db, single_deck_with_cards):
        ...
        empty_deck = await DeckManager.create_deck_record(empty_deck_create)

        result = await DeckCardManager.gets_for_deck(empty_deck.id)
    ...
    async def test_get_cards_for_nonexistent_deck(self, db, single_deck_with_cards):
        ...
        result = await DeckCardManager.gets_for_deck(non_existent_deck_id)
...
@pytest.mark.skip("standard")
@pytest.mark.asyncio
class TestDeckCardManagerUpdate:
    """Integration tests for DeckCardManager update operations."""
    ...
    async def test_update_card_quantity_success(self, db, single_deck_card):
        ...
        result = await DeckCardManager.update_quantity_in_deck(
...
@pytest.mark.skip("standard")
@pytest.mark.asyncio
class TestDeckCardManagerRemove:
    """Integration tests for DeckCardManager remove operations."""
    ...
    async def test_remove_card_from_deck_success(self, db, single_deck_card):
        ...
        result = await DeckCardManager.remove_card_from_deck(test_deck_card.id)

        assert result is True

        # Verify card is actually removed
        remaining_cards = await DeckCardManager.gets_for_deck(test_deck.id)
    ...
    async def test_remove_nonexistent_deck_card(self, db, single_deck_card):
        ...
        result = await DeckCardManager.remove_card_from_deck(non_existent_id)
    ...
    async def test_remove_card_multiple_times(self, db, single_deck_card):
        ...
        result1 = await DeckCardManager.remove_card_from_deck(test_deck_card.id)
        assert result1 is True

        # Second removal should fail
        result2 = await DeckCardManager.remove_card_from_deck(test_deck_card.id)
    ...
    async def test_remove_card_preserves_other_cards(
        ...
        another_card = await CardManager.create(card_create)

        deck_card_create = DeckCardCreate(card_id=another_card.id, quantity=2)
        another_deck_card = await DeckCardManager.add_card_to_deck(
...
@pytest.mark.asyncio
class TestDeckCardManagerEdgeCases:
    """Integration tests for DeckCardManager edge cases and error conditions."""
    ...
    async def test_add_card_quantity_accumulation_multiple_additions(
        ...
        result3 = await DeckCardManager.add_card_to_deck(test_deck.id, deck_card_create)
    ...
    async def test_deck_card_unique_constraint(self, db, single_deck_and_cards):
        ...
        cards_in_deck = await DeckCardManager.gets_for_deck(test_deck.id)
    ...
    async def test_operations_with_nonexistent_card_id(self, db, single_deck_and_cards):
        ...
        with pytest.raises(Exception):  # Could be IntegrityError or similar
            await DeckCardManager.add_card_to_deck(test_deck.id, deck_card_create)
    ...
    async def test_operations_with_nonexistent_deck_id(self, db, single_deck_and_cards):
        ...
        # This should fail due to foreign key constraint
        with pytest.raises(Exception):  # Could be IntegrityError or similar
            await DeckCardManager.add_card_to_deck(
                non_existent_deck_id, deck_card_create
            )
