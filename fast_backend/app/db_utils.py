...
class DatabaseUtils:
    ...
    async def export_data(self, filename: str = None):
        ...
        print(f"✅ Exported {len(export_data['users'])} users, {len(export_data['cards'])} cards, {len(export_data['decks'])} decks")
    ...
    async def show_sample_data(self):
        ...
        if decks:
            ...
            for deck in decks:
                ...
                desc_preview = deck.description[:50] + "..." if len(deck.description) > 50 else deck.description
    ...
    async def reset_database(self):
        ...
        try:
            from app.quick_populate import quick_populate
            print("🚀 Running quick population...")
            await quick_populate()
        except ImportError:
            print("⚠️  quick_populate not available, database cleared but not repopulated")
...
async def main():
    ...
    parser.add_argument("--export", type=str, nargs="?", const="auto", help="Export data to JSON file")
    parser.add_argument("--sample", action="store_true", help="Show sample data")
    parser.add_argument("--reset", action="store_true", help="Reset database and populate with test data")
