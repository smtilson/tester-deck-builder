...
from .db.config import init_db, close_db, MODEL_PATHS
...
SAMPLE_CARDS_DATA = [
    # Creatures
    {"name": "Lightning Drake", "text": "Flying. When Lightning Drake enters play, deal 2 damage to any target."},
    {"name": "Forest Guardian", "text": "Defender. Regenerate: Pay 1 green mana."},
    {"name": "Goblin Warrior", "text": "Haste. Cannot block."},
    {"name": "Ancient Dragon", "text": "Flying, Trample. When Ancient Dragon attacks, draw a card."},
    {"name": "Knight of Honor", "text": "First Strike, Vigilance. Protection from black."},
    {"name": "Shadow Assassin", "text": "Stealth. When Shadow Assassin deals damage, target opponent discards a card."},
    {"name": "Crystal Golem", "text": "Artifact Creature. Gets +1/+1 for each artifact you control."},
    {"name": "Fire Elemental", "text": "When Fire Elemental enters play, deal 3 damage to target creature."},
    {"name": "Sea Serpent", "text": "Can only attack if defending player controls an island."},
    ...
    {"name": "Fireball", "text": "Deal X damage to any target, where X is the amount of mana spent."},
    ...
    # Artifacts
    {"name": "Sword of Power", "text": "Equipped creature gets +2/+2. Equip cost: 2 mana."},
    {"name": "Mana Crystal", "text": "Tap: Add 1 colorless mana to your mana pool."},
    {"name": "Shield of Defense", "text": "Equipped creature gets +0/+3 and defender. Equip cost: 1 mana."},
    {"name": "Ring of Wisdom", "text": "Equipped creature can draw an extra card each turn. Equip cost: 3 mana."},
    {"name": "Boots of Speed", "text": "Equipped creature has haste. Equip cost: 1 mana."},
    ...
    {"name": "Nature's Bounty", "text": "At the beginning of your turn, gain 1 life for each creature you control."},
    ...
    {"name": "Crystal Caves", "text": "Tap: Add 1 mana of any color. Enters play tapped."},
    {"name": "Mystic Temple", "text": "Tap: Add 1 mana of any color. Pay 3: Scry 1."},
]

# Sample deck themes and names
DECK_THEMES = [
    {"name": "Lightning Storm", "description": "Fast aggressive deck focused on dealing quick damage with lightning spells and creatures."},
    {"name": "Forest Defense", "description": "Defensive strategy using nature's guardians and healing magic."},
    {"name": "Shadow Control", "description": "Control the battlefield with dark magic and mind manipulation."},
    {"name": "Crystal Power", "description": "Artifact-based deck leveraging powerful equipment and constructs."},
    {"name": "Dragon's Fury", "description": "High-cost, high-reward deck centered around powerful dragons."},
    {"name": "Vampire Swarm", "description": "Aggressive deck using vampire synergies and life-drain abilities."},
    {"name": "Knight's Honor", "description": "Midrange deck focused on honorable knights and protection spells."},
    {"name": "Elemental Chaos", "description": "Unpredictable deck mixing various elemental creatures and spells."},
    {"name": "Sea Mastery", "description": "Blue-focused deck controlling the game through counterspells and sea creatures."},
    {"name": "Goblin Rush", "description": "Ultra-fast aggro deck overwhelming opponents with goblin creatures."},
    {"name": "Healing Sanctuary", "description": "Control deck focused on gaining life and protecting key creatures."},
    {"name": "Ancient Wisdom", "description": "Spell-heavy deck drawing cards and casting powerful magic."},
    {"name": "Fire & Steel", "description": "Combination of aggressive creatures and powerful artifacts."},
    {"name": "Nature's Wrath", "description": "Green ramp deck playing large creatures and overwhelming spells."},
    {"name": "Darkness Rising", "description": "Black deck using sacrifice and dark rituals for powerful effects."},
]
...
USER_NAMES = [
    ...
    "Quinn", "River", "Sage", "Taylor", "Unique", "Val", "Winter", "Xander",
    "Yarrow", "Zara", "Avery", "Bay", "Carmen", "Dakota", "Eden", "Flynn"
]

ADJECTIVES = [
    "Swift", "Mighty", "Clever", "Bold", "Fierce", "Wise", "Noble", "Brave",
    "Cunning", "Radiant", "Shadow", "Storm", "Frost", "Flame", "Stone", "Wind",
    "Iron", "Silver", "Golden", "Crystal", "Mystic", "Ancient", "Elder", "Prime"
]
...
class DataGenerator:
    ...
    async def generate_cards(self, count: int = None) -> List[Card]:
        ...
        # Create cards
        for i in range(count):
            card_data = card_pool[i] if i < len(card_pool) else random.choice(SAMPLE_CARDS_DATA)
    ...
    async def generate_decks(self, count: int = 15) -> List[Deck]:
        ...
        for i in range(count):
            ...
            deck = await Deck.create(
                ...
                owner_id=int(owner.id.hex[:6], 16)  # Convert UUID to int for owner_id
            )
            decks.append(deck)
    ...
    async def generate_deck_cards(self) -> List[DeckCard]:
        ...
        if not self.created_decks or not self.created_cards:
            raise ValueError("Decks and cards must be created before deck-card relationships")
        ...
        for deck in self.created_decks:
            ...
            selected_cards = random.sample(self.created_cards, min(deck_size, len(self.created_cards)))
    ...
    async def generate_statistics(self):
        ...
        for deck in decks:
            ...
            print(f"   Deck: {deck.name} ({card_count} cards) - {deck.description[:50]}...")


async def main():
    """Main function to generate database content."""
    parser = argparse.ArgumentParser(description="Generate dummy data for the card game database")
    parser.add_argument("--users", type=int, default=15, help="Number of users to create")
    parser.add_argument("--cards", type=int, default=None, help="Number of cards to create (default: all sample cards)")
    parser.add_argument("--decks", type=int, default=20, help="Number of decks to create")
